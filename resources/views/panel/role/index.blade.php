                        ...
                        <form action="/roles" method="post">
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <label> نقش ها</label>
                                        <select name="permissions[]" class="js-example-basic-single" multiple required>
                        ...
                        @foreach($rows as $row)
                            ...
                            <td style="text-align: center">
                                @foreach($row->permissions as $permission)
                                    <span class="btn btn-sm btn-info">{{$permission->label}}</span>
                                @endforeach
                            </td>
                            <td style="text-align: center">
                                ...
                                <div class="modal fade " id="edit{{$row->id}}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog modal-xl" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">ویرایش نق</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="/roles/{{$row->id}}" method="post">
                                                @csrf
                                                {{csrf_field()}}
                                                {{method_field('PATCH')}}
                                                <input name="type" hidden value="mahd">
                                                <div class="modal-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-3">
                                                            <label> نقش ها</label>
                                                            <select name="permissions[]" class="js-example-basic-single"
                                                                    multiple required>
                                                                @foreach($permissions as $p)
                                                                    <option
                                                                        @if(in_array($p->id,$row->permissions->pluck('id')->toArray())) selected
                                                                        @endif value="{{$p->id}}">{{$p->label}}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                        </div>
