                    ...
                    <tr>
                        ...
                        @foreach($rows as $row)
                            ...
                            <td style="text-align: center">{{$row->name}} {{$row->family}}</td>
                                <td style="text-align: center">{{minutes_to_time($row->mahd_roolcall_sum_sum)}}</td>
                            ...
                            <td style="text-align: center">
                                    ...
                                    <button type="button" class="btn btn-success btn-sm" onclick="enter({{$row->id}})">ورود</button>


                                    <button type="button" class="btn btn-success btn-sm" onclick="exit({{$row->id}})">خروج</button>


                                    <button type="button" class="btn btn-danger btn-sm" onclick="absent({{$row->id}})">غایب</button>
    ...
    function enter(id) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: "{{  url('/planes/rollcall/enter/')  }}" + '/' + id,
                        type: "GET",
    ...
    function exit(id) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: "{{  url('/planes/rollcall/exit/')  }}" + '/' + id,
                        type: "GET",
    ...
    function absent(id) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: "{{  url('/planes/rollcall/absent/')  }}" + '/' + id,
                        type: "GET",
