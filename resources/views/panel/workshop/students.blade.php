            ...
            <div class="modal fade " id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog modal-xl" role="document">
                    <div class="modal-content">
                        ...
                        <form action="/workshop/add_student" method="post">
                            @csrf
                    ...
                    <tr>
                        ...
                        @foreach($rows as $row)
                            ...
                            <td style="text-align: center">
                                <img @if($row->user->image) src="/user/{{$row->user->image}}"
                            ...
                            <td style="text-align: center">{{$row->user->father_name}}</td>
                            <td style="text-align: center">{{$row->user->mother_name}}</td>
                            <td style="text-align: center">{{$row->user->father_mobile}}</td>
                            <td style="text-align: center">{{$row->user->mother_mobile}}</td>
                            <td style="text-align: center">{{number_format($row->remaining)}}</td>

                            <td style="text-align: center">
                                <button class="btn  btn-danger" onclick="deleteData({{$row->id}})"><i
                                        class="fa fa-trash"></i> &nbsp; حذف از کارگاه
                                </button>
                            </td>
                    </tr>
                    <?php $idn = $idn + 1 ?>
    ...
    function deleteData(id) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: "{{  url('/workshop/delete_student/')  }}" + '/' + id,
                        type: "GET",
